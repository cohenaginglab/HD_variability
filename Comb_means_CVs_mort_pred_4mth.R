#######################################################################################################
#####      This code is for the figure entitled "Effect of combining biomarker means and CVs      ##### 
#####   on prediction of mortality for PC1, PC2, PC7, and CVPC3 using the 4-month variable list"  #####
#######################################################################################################
...
write.csv(res, "Comb_means_CVs_mort_pred_4mth.csv", row.names = F)

#### Plot
tiff("Comb_means_CVs_mort_pred_4mth.tiff", width = 180, height = 270, "mm", res = 300, compression = "lzw")
...
ylims <- c(13, 13, 7, 9, 5)
yaxps <- list(c(1, 13, 6), c(1, 13, 6), c(1, 7, 6), c(1, 9, 4), c(1, 5, 4))
...
for (pc in 1:length(pcs)) {
  ...
  plot(1:10, 1:10, type = "n", xlim = c(1, length(selected.biomarkersHD)), 
       xlab = "# of PCs included", ylim = c(1, ylims[pc]),
       ylab = "HR95", frame.plot = F, cex.axis = 0.9, xaxp = c(2, 16, 7), yaxp = yaxps[[pc]])
  ## Add grid 
  abline(h = seq(2, 12, 2), col = "grey85")
  abline(h = seq(1, 13, 2), col = "grey85", lwd = 2)
