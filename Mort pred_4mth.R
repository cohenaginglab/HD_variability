#######################################################################################################
#####            This code is for the figure entitled "Prediction of death by biomarker           ##### 
#####                   mean and variance indices for the 4-month variable list"                  #####
#######################################################################################################
...
tiff("Mort pred_4mth.tiff", width = 180, height = 180, "mm", res = 300, compression = "lzw")
...
write.csv(res_2wks, "Mort pred_4mth_panelsA_B.csv", row.names = F)
...
for (um in 1:2) {
  ...
  plot(1:10, 1:10, type = "n", xlim = x_range, ylim = c(0, 15.5), 
  ...
  text_labels <- list("uni" = vars.nms, 
                      "multi" = paste("PC", 1:(nrow(res_um) / 2), sep = ""))
  
  mtext(text = c("PH assumption", "P-value"), side = 3, at = -2,
  ...
  for (ms in 1:2) {
    ...
    mtext(text = c("Means", "CVs")[ms], side = 3, at = c(-3.25, -0.75)[ms],
          line = -0.25, cex = 0.6, adj = 0.5, col = myPal1[ms], font = 2)
    text(x = rep(c(-3.25, -0.75)[ms], nrow(res_ms)), y = (nrow(res_ms) - 1):0,
  ...
  if (um==2) {
    legend(x = 3.2, y = 3, legend = c("Levels (means)", "Variability (CVs)"), 
...
write.csv(res_all, "Mort pred_4mth_panelC.csv", row.names = F)
