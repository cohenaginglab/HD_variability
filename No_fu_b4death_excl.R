#######################################################################################################
#####       This code is for the figure entitled "Prediction of death by biomarker mean and       ##### 
#####    variance indices, excluding individuals who died but were not followed-up until death"   #####
#######################################################################################################
...
# Exclude individuals for whom we have a death date but who were not follow-up until death (1 month before death)
clean.HD2 <- ddply(clean.HD, .(id_no), no.fu, time = 1)
clean.HD2 <- clean.HD2[-which(!is.na(clean.HD2$date_death) & clean.HD2$fu.until.death==0), ]
length(unique(clean.HD$id_no)) - length(unique(clean.HD2$id_no))
nrow(clean.HD) - nrow(clean.HD2)
# Excluded 112 patients (4436 obs.)
...
tiff("No_fu_b4death_excl.tiff", width = 180, height = 180, "mm", res = 300, compression = "lzw")
...
write.csv(res_2wks, "No_fu_b4death_excl_panelsA_B.csv", row.names = F)
...
for (um in 1:2) {
  ...
  for (ms in 1:2) {
    ...
    mtext(text = c("Means", "CVs")[ms], side = 3, at = c(-6, -2)[ms],
          line = -0.25, cex = 0.6, adj = 0.5, col = myPal1[ms], font = 2)
    text(x = rep(c(-6, -2)[ms], nrow(res_ms)), y = (nrow(res_ms) - 1):0,
...
write.csv(res_all, "No_fu_b4death_excl_panelC.csv", row.names = F)
...
## Add grid 
abline(h = c(0.5, 0.6, 0.7, 0.8, 0.9), col = "grey85", lwd = 2)
