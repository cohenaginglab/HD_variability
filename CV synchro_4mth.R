#######################################################################################################
#####            This code is for the figure entitled "Physiological variability shows            ##### 
#####       a strong coordinated signal distributed evenly across all 16 measured biomarkers"     #####
#######################################################################################################
library(ggplot2)
library(ggpubr)
...
plot_b <- ggplot(pc.contrib, aes(x = Time, y = Contribution)) +
  ...
  theme(axis.title.y = element_text(size = 10, face = "bold"), axis.text.x = element_text(size = 7), 
        ...
        legend.position = "right", legend.margin = margin(0, 0, 0, 0.05), legend.key.size =  unit(3, "mm"),
...
####### Save Figure ########
tiff("CV synchro_4mth.tiff", 210, 150, "mm", res = 600, compression = "lzw", bg = "transparent")
